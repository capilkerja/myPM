<x-filament-panels::page>
    ...
    <div class="mb-8" x-data="{ showProjectSelector: false }">
        <!-- Toggle Button -->
        <div class="mb-4">
            <button @click="showProjectSelector = !showProjectSelector"
                ...
                <svg class="w-4 h-4 transition-transform" :class="{ 'rotate-180': showProjectSelector }" fill="none"
        ...
        <div x-show="showProjectSelector" x-transition:enter="transition ease-out duration-200"
    ...
    @if ($selectedProject)
        <div x-data="{
        ...
        }" x-init="init()" @ticket-moved.window="init()"
            @ticket-updated.window="init()" @refresh-board.window="init()"
            ...
            @if (!$this->canMoveTickets())
                <div class="flex justify-center mb-4">
                    <div
                        class="inline-flex items-center gap-2 px-4 py-2 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg text-amber-800 dark:text-amber-200">
            ...
            <div class="inline-flex gap-4 pb-2 min-w-full">
                @foreach ($ticketStatuses as $status)
                    <div class="status-column rounded-xl border border-gray-200 dark:border-gray-700 flex flex-col bg-gray-50 dark:bg-gray-900"
                        ...
                        <div class="px-4 py-3 rounded-t-xl border-b border-gray-200 dark:border-gray-700 flex-shrink-0"
                            ...
                            <div class="flex items-center justify-between">
                                <h3 class="font-medium flex items-center gap-2"
                                    ...
                                    @if ($status->is_completed)
                                        <div class="flex items-center justify-center w-6 h-6 bg-green-500 rounded-full border-2 border-white shadow-lg"
                                            title="Completed Status">
                                            <svg class="w-3 h-3 text-white font-bold" fill="currentColor"
                                                viewBox="0 0 20 20">
                                    ...
                                    <div x-show="open" x-transition:enter="transition ease-out duration-100"
                                        ...
                                        <div class="p-2">
                                            <div
                                                ...
                                                <span class="text-sm font-medium text-gray-900 dark:text-white">Sort
                                                    list</span>
                        ...
                        <div class="p-3 flex flex-col gap-3 flex-1 overflow-y-auto"
                            style="max-height: calc(100% - 60px);" x-data="{ visibleTickets: 10, totalTickets: {{ $status->tickets->count() }} }" x-init="$nextTick(() => { $el.addEventListener('scroll', () => { if ($el.scrollTop + $el.clientHeight >= $el.scrollHeight - 100 && visibleTickets < totalTickets) { visibleTickets = Math.min(visibleTickets + 10, totalTickets); } }); })">
                                ...
                                <div class="ticket-card bg-white dark:bg-gray-800 p-3 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 cursor-move"
                                    ...
                                    <div class="flex justify-between items-center mb-2">
                                        ...
                                        <div class="flex items-center gap-1">
                                            @if ($ticket->priority)
                                                <span
                                                    class="text-xs px-1.5 py-0.5 rounded whitespace-nowrap text-white font-medium"
                                    ...
                                    <h4 class="font-medium text-gray-900 dark:text-white mb-2">{{ $ticket->name }}
                                    </h4>
                                        ...
                                        @if ($ticket->assignees->isNotEmpty())
                                            <div class="flex flex-wrap gap-1 max-w-[180px]">
                                                @foreach ($ticket->assignees->take(2) as $assignee)
                                                    <div
                                                        class="inline-flex items-center px-2 py-1 rounded-full bg-primary-100 dark:bg-primary-900/40 text-primary-700 dark:text-primary-300 gap-1">
                                                        <span
                                                        ...
                                                        <span
                                                            class="text-xs font-medium truncate">{{ \Illuminate\Support\Str::limit($assignee->name, 8) }}</span>
                                                    </div>
                                                @endforeach
                                                @if ($ticket->assignees->count() > 2)
                                                    <div
                                                        ...
                                                        <span
                                                            class="text-xs font-medium">+{{ $ticket->assignees->count() - 2 }}</span>
                                                    </div>
                                                @endif
                                            </div>
                                        @else
                            ...
                            @else
                                ...
                                <div x-show="visibleTickets < totalTickets"
                                    class="flex items-center justify-center py-4 text-gray-500 dark:text-gray-400 text-sm">
